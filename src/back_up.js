...
export default class App extends Lightning.Component {
    ...
    x: 20, y: 20,
      Buttons: {
        LeftButton: {
          ...
          Label: {
            x: 75, y: 22, mount: .5, textColor: "red", text: { fontSize: 20, text: 'Left' }
          }
        },
        RightButton: {
          x: 200, color: 0xff1f1f1f,
          texture: lng.Tools.getRoundRect(150, 40, 4),
          Label: {
            x: 75, y: 22, mount: .5, textColor: "white", text: { fontSize: 20, text: 'Right' }
          }
        }
      },
      RectangleDefault: {
          x: 100, y: 100, w: 200, h: 100, rect: true,  color: 0xFF1C27bC
        },
  ...
  _init() {
    // Pin.show().then( => to show pin dialog on screen
      // Log.info("My AppName is :: " + Metadata.appName())
    // )
    this.buttonIndex = 0
...
_handleDown() {
    this._setState('RectangleDefault')
}
static _states() {
  return [
